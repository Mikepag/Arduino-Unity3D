...
public class diskRotationP1 : MonoBehaviour
{
    ...
    IEnumerator RotateObject(int directionSteps)
    {
        ...
        if (isStationary == 1 && angle >= 175 && angle <= 185 && ((resBtnClicked - previous_resBtnClicked) == 0))    // If the current angle is between 175 and 185 and the button has not been clicked yet to start the next round...
        {
