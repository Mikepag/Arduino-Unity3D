...
public class timerP1 : MonoBehaviour
{
    ...
    private int roundNum;                       // Used to save the value of the roundNum variable from the restartP1.cs script.
    ...
    private int previous_resBtnClicked;         // Used to save the previous value of the resBtnClicked variable from the restartP1.cs script. (I use it to know when the restart button gets clicked by compairing it to the current resBtnClicked).
    ...
    void Update()
    {
        ...
        roundNum = Disk.GetComponent<restartP1>().roundNum;                // Getting the value of roundNum from the restartP1.cs script.

        previous_resBtnClicked = resBtnClicked; //Save the previous frame's resBtnClicked value.
        resBtnClicked = Disk.GetComponent<restartP1>().resBtnClicked;      // Getting the current value of resBtnClicked from the restartP1.cs script.
        ////////////


        if (goalReached == 1 && (resBtnClicked - previous_resBtnClicked) == 0)  // If the user reached the goal and has not clicked the restart button yet (in the current round)...
        {
        ...
        if ((resBtnClicked - previous_resBtnClicked) == 1)             // If the restart button got clicked (in the current round)...
        {
