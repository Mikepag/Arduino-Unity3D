...
public class ghostRotationDamped : MonoBehaviour
{
    ...
    public float initialAngleOfDampedRotation;
    ...
    void Update()   // Update is called once per frame
    {
        ...
        {                                                               // If the input value is between the boundaries...
            ...
            angleOfRotation = GetAngleOfRotation();
        ...
        else                                                            // Else, if the input is out of bounds...
        {
            ...
            {
                initialAngleOfDampedRotation = angleOfRotation;
            }

            if (timestepsSinceOutOfBounds < 20)
            {
                DampedGhostRotation(initialAngleOfDampedRotation, timestepsSinceOutOfBounds);
            }
    ...
    {
        averagePreviousPosition = 0;
        if (currentArrSize > 0) // To avoid the division with zero when the array is empty.
        {
    ...
    void GhostRotation(float angleOfRotation)                                                 // Call GhostRotation() to rotate the Ghost.
    {
        // --- GHOST ROTATION ---
        Ghost.transform.Rotate(0, 0, angleOfRotation);                     // Rotate the Ghost in the Z-Axis in the direction and degrees provided by angleOfRotation.
    }


    //__________________________________________________DAMPEDGHOSTROTATION():__________________________________________________
    void DampedGhostRotation(float initialAngleOfDampedRotation, int timestepsSinceOutOfBounds)
    {
        angleOfRotation = initialAngleOfDampedRotation * (100 - (5 * (1 + timestepsSinceOutOfBounds))) / 100;   // Remove 5% of the initial angle of rotation for every elapsed timestep, so the rotational speed decreases.

        // --- GHOST ROTATION ---
        Ghost.transform.Rotate(0, 0, angleOfRotation);
    }
