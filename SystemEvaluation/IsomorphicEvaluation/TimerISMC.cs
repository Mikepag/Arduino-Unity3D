...
public class TimerISMC : MonoBehaviour
{
    ...
    private string filename = "LogTimes_Isomorphic.txt";    // External file where the elapsed time of each round is beeing written.
    ...
    void Update()
    {
        goalReached = Disk.GetComponent<DiskRotationISMC>().goalReached;          // Getting the value of goalReached from the diskRotationISMC.cs script.
        roundNum = Disk.GetComponent<RestartISMC>().roundNum;                     // Getting the value of roundNum from the restartISMC.cs script.
